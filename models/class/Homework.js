...
class Homework {
  ...
  async isExistHw(hwID) {
    try {
      ...
      var ret = await conn.query(
        "select * from class_project where classProjectID = ?",
        [hwID]
      );
      if (ret[0].length == 0) {
        return 0;
  ...
  async getAllHw(classID, stuID) {
    try {
      var conn = await pool.getConnection();
      var ret = await conn.query(
        "select * from class_project left outer join class_project_score on class_project.classProjectID = class_project_score.classProjectID \
            where class_project.classID = ? and (class_project_score.studentID is null or class_project_score.studentID = ?)",
        [classID, stuID]
      );
      if (ret[0].length == 0) {
        return null;
  ...
  async getHwInfo(hwID, stuID) {
    try {
      ...
      if (stuID != undefined) {
        ...
        var ret = await conn.query(
          "select * from class_project left outer join class_project_score on class_project.classProjectID = class_project_score.classProjectID where class_project.classProjectID = ?",
          [hwID]
        );
      }
