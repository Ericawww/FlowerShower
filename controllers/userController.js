...
exports.userSettings = async (req, res) => {
    ...
    if (req.session.token == null) {
        res.send(
            "<script>alert('您暂无权限访问该页面，请先登录！'); window.location.href='/';</script>"
        );
        return;
    }
    var token = req.session.token;
    token.birth = utils.dateFormat(token.birth, "yyyy-MM-dd");
    if (parseInt(token.userType) == config.TYPE_TEACHER) {
        //如果身份是教师还需要传送教师个人主页
...
exports.userChangeImage = async (req, res) => {
    ...
    try {
        var base64Data = decodeURIComponent(req.body.imgBase64)
        ...
        fs.writeFile(newFileName, dataBuffer, async err => {
            if (err) {
                ...
                try {
                    //修改数据库中的路径
                    await User.prototype.updatePhotoSrc(
                ...
                try {
                    ...
                    if (req.session.token.userPhoto) {
                        var oldFileName = path.join(
                            __dirname,
                            "../",
                            "/public/",
                            req.session.token.userPhoto
                        );
...
exports.updateUserPwd = async (req, res) => {
    ...
    var checkFlag = await User.prototype.login(
        req.session.token.userID,
        req.body.oldPwd
    );
...
exports.getGrade = async (req, res) => {
    ...
    for (var i = 0; i < firstPartResult.length; i++) {
        var gradeWeight = await User.prototype.getGradeWeight(firstPartResult[i].classID);
        ...
        avgGrade += parseFloat(totalGrade[i]);
    ...
    if (firstPartResult.length != 0) {
        avgGrade = parseFloat(avgGrade / firstPartResult.length).toFixed(2);
    }
    res.render('users/userGrade', {
        firstPartResult: firstPartResult,
