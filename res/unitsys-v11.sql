...
-- ----------------------------
INSERT INTO `problem_bank` VALUES ('1', '031E0031', 'G13最憨批的是？', '贺婷婷', '胡图图', 'HTT', '张佳瑶', '15', 'T0001');
...
DROP TRIGGER IF EXISTS `fullMark_add`;
DELIMITER ;;
CREATE TRIGGER `fullMark_add` AFTER INSERT ON `project_problem` FOR EACH ROW begin
	update class_project set fullMark = fullMark + new.score where classProjectID = new.classProjectID;
end
;;
DELIMITER ;
DROP TRIGGER IF EXISTS `fullMark_delete`;
DELIMITER ;;
CREATE TRIGGER `fullMark_delete` AFTER DELETE ON `project_problem` FOR EACH ROW begin
	update class_project set fullMark = fullMark - old.score where classProjectID = old.classProjectID;
end
;;
DELIMITER ;
